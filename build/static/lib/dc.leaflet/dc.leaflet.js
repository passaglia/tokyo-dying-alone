...
// Sam version extended  to allow passing arguments to the geojson
dc_leaflet.choroplethChartX = function(parent, chartGroup) {
    var _chart = dc_leaflet.leafletBase(dc.ColorMixin(dc.MarginMixin));
    ...
    var _featureOptionsHighlighted = {
        'opacity':1,
        'weight':5
    };
    ...
    var _featureStyle = function(feature) {
        ...
        if (v && v.d) {
            options.fillColor=_chart.getColor(v.d, v.i);
            if (_chart.filters().indexOf(v.d.key) !== -1) {
                // Sam: getting the additional features for highlits
                var highlightedOptions = _chart.featureOptionsHighlighted();
                highlightedOptions = JSON.parse(JSON.stringify(highlightedOptions));
                // Sam: destructuring assignment
                options = {
                    ...options,
                    ...highlightedOptions
                };
    ...
    // Sam edit
    var _forEachFeature = function(feature, layer) {
        return undefined;
    };
    ...
    _chart.featureOptionsHighlighted = function(_) {
        if (!arguments.length) {
            return _featureOptionsHighlighted;
        }
        _featureOptionsHighlighted = _;
        return _chart;
    };
    ...
    // Sam edit
    _chart.forEachFeature = function(_) {
        if (!arguments.length) {
            return _forEachFeature;
        }
        _forEachFeature= _;
        return _chart;
    };
    ...
    var processFeatures = function (feature, layer) {
        ...
        if (v && v.d) {
            ...
            if (_chart.brushOn())
                layer.on("click", selectFilter);
            // Sam Edit    
            _chart.forEachFeature()(feature, layer);
        }
    };
